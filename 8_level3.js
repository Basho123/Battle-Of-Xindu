...
kosPukacSpeed = 1;
  ...
  fill(255);
  textAlign(LEFT);
  text('enemies remaining: ' + (muvi3.length), 10, height - 2);
...
///////////ASTEROID POZADINA//////////
  ...
  asteroid[a].show(asteroidTexture3);
  asteroid[a].move();
...
if (muvi3.length === 0){
  for (let q = bossMuva3.length - 1; q >= 0; q--) {

     push();
    let hBar =  map (boss3Health, 0, boss3HealthBar, 0, width/3)
  ...
  // fill(255);
  // textAlign(CENTER);
  // text('Combined boss health: ' + (boss3Health), width/2, height - 20);

    for (let i = random(0, enemyPukacRate * 0.05); i < 1; i++) {
      let e = new EnemyPukac(bossMuva3[q].x, bossMuva3[q].y);
    ...
    for (let i = random(0, enemyPukacRate * 0.08); i < 1; i++) {
      let e = new EnemyPukac(bossMuva3[q].x, bossMuva3[q].y);
      enemyPukac.push(e);
 ...
 ////////muvi show   
    ...
    for (let w = muvi3.length - 1; w >= 0; w--) {

    muvi3[w].show();
    muvi3[w].move();

    for (let i = random(0, enemyPukacRate * 1); i < 1; i++) {
      let e = new EnemyPukac(muvi3[w].x, muvi3[w].y);
      enemyPukacKos.push(e);
  ...
  for (let i = pukac.length - 1; i >= 0; i--) {
   ...
   // pointLight(255, 50, 0, pukac[i].x,-250, pukac[i].y-height);
    ...
    for (var g = 0; g < muvi3.length; g++) {
      if (pukac[i].hits(muvi3[g])) {
    ...
    for (var b = 0; b < bossMuva3.length; b++) {
      if (pukac[i].hits(bossMuva3[b])) {
        
        if(upgraded === 0 && upgraded <5){
        boss3Health = boss3Health - 5;
        }
        
          if(upgraded === 5 && upgraded <10){
        boss3Health = boss3Health - 10;
        }
        
         if(upgraded >= 10){
        boss3Health = boss3Health - 15;
        }
        if (boss3Health <= 1) {
          bossMuva3[b].kill();
        }
        ...
        hitSound.play();
        hitSound.setVolume(0.5)
        if (pukac[i].hits(bossMuva3[b]) && boss3Health <= 0 ){
  ...
  for (let d = 0; d < bossMuva3.length; d++) {
    if (bossMuva3[d].toDelete) {
      bossMuva3.splice(d, 1)
    }
  }
